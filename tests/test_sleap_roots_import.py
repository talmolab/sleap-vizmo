...
class TestSleapRootsImports:
    ...
    def test_trait_pipelines_module(self):
        ...
        try:
            ...
            assert hasattr(trait_pipelines, "MultipleDicotPipeline"), (
                f"MultipleDicotPipeline not found. " f"Available: {sorted(available)}"
            )

            # Check if the pipeline has the method we need
            pipeline = trait_pipelines.MultipleDicotPipeline()
            assert hasattr(
                pipeline, "compute_multiple_dicots_traits"
    ...
    def test_alternative_dicot_functions(self):
        ...
        pytest.skip("Skipping recursive module search test that may hang")
        import sleap_roots
    ...
    def test_sleap_roots_documentation(self):
        ...
        if sleap_roots.__doc__:
            print(
                sleap_roots.__doc__[:500] + "..."
                if len(sleap_roots.__doc__) > 500
                else sleap_roots.__doc__
            )
        ...
        try:
            ...
            if trait_pipelines.__doc__:
                print(
                    trait_pipelines.__doc__[:500] + "..."
                    if len(trait_pipelines.__doc__) > 500
                    else trait_pipelines.__doc__
                )
        except ImportError:
    ...
    def test_complete_sleap_roots_workflow(self):
        ...
        try:
            ...
            ), "Failed to create MultipleDicotPipeline instance"
            ...
            print(
                f"   - compute_multiple_dicots_traits: {pipeline.compute_multiple_dicots_traits}"
            )
