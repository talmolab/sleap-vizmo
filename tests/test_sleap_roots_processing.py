...
class TestCreateExpectedCountCsv:
    ...
    def test_basic_functionality(self, tmp_path):
        ...
        df, csv_path_returned = create_expected_count_csv(
            mock_series, series_data, tmp_path
        )
    ...
    def test_genotype_extraction(self, tmp_path):
        ...
        series_data = {series.series_name: {"primary_path": "test.primary.slp"}}
...
class TestMoveOutputFilesToDirectory:
    ...
    def test_move_files(self, tmp_path):
        ...
        with patch("glob.glob") as mock_glob:
            ...
            moved = move_output_files_to_directory(
                output_dir, ["*_all_plants_traits.csv"]
            )
...
class TestCombineTraitCsvs:
    ...
    def test_combine_csvs(self, tmp_path):
        ...
        for i in range(3):
            df = pd.DataFrame({"trait1": [1, 2, 3], "trait2": [4, 5, 6]})
...
class TestMergeTraitsWithExpectedCounts:
    ...
    def test_custom_timestamp(self, tmp_path):
        """Test using custom timestamp."""
        traits_df = pd.DataFrame({"series_name": ["series1"], "trait1": [10.0]})
    ...
    def test_missing_series_in_expected_counts(self, tmp_path):
        ...
        traits_df = pd.DataFrame(
            {"series_name": ["series1", "series_unknown"], "trait1": [10.0, 20.0]}
        )
...
class TestCreateProcessingSummary:
    ...
    def test_complete_summary(self, tmp_path):
        ...
        # Create expected count DataFrame
        expected_count_df = pd.DataFrame({"number_of_plants_cylinder": [7, 8, 7, 8, 8]})
...
class TestSaveNotebookSnapshot:
    """Test save_notebook_snapshot function."""

    def test_save_existing_notebook(self, tmp_path):
        """Test saving an existing notebook."""
        ...
        # Test without HTML (to avoid nbconvert dependency in tests)
        result = save_notebook_snapshot(output_dir, test_notebook, save_html=False)
    ...
    def test_save_with_custom_name(self, tmp_path):
        """Test saving with custom notebook name."""
        # Create test notebook
        test_notebook = tmp_path / "test.ipynb"
        test_notebook.write_text('{"cells": []}')
        ...
        result = save_notebook_snapshot(
            output_dir,
            test_notebook,
            notebook_name="custom_snapshot.ipynb",
            save_html=False,
        )

        assert result is not None
        assert result.name == "custom_snapshot.ipynb"

    def test_notebook_not_found(self, tmp_path):
        """Test handling when notebook is not found."""
        output_dir = tmp_path / "output"
        output_dir.mkdir()

        # Try with non-existent path
        result = save_notebook_snapshot(output_dir, tmp_path / "nonexistent.ipynb")
        assert result is None

    def test_default_notebook_search(self, tmp_path, monkeypatch):
        """Test searching for default notebook in common locations."""
        # Change to temp directory
        monkeypatch.chdir(tmp_path)

        # Create notebook in current directory
        notebook = tmp_path / "sleap_roots_processing.ipynb"
        notebook.write_text('{"cells": []}')

        output_dir = tmp_path / "output"
        output_dir.mkdir()

        # Should find notebook without specifying path
        result = save_notebook_snapshot(output_dir, save_html=False)
        assert result is not None
        assert result.exists()

    def test_save_with_html(self, tmp_path):
        """Test saving with HTML export (if nbconvert available)."""
        # Create a mock notebook file
        notebook_content = {
            "cells": [
                {
        ...
        result = save_notebook_snapshot(output_dir, test_notebook, save_html=True)

        # Verify notebook was created
        assert result is not None
        assert result.exists()

        # Check if HTML was created (it might fail if nbconvert is not available)
        html_path = output_dir / "sleap_roots_processing_notebook_snapshot.html"
        # We don't assert it exists since nbconvert might not be available in test env

    def test_save_with_suffix(self, tmp_path):
        """Test saving notebook with suffix."""
        # Create test notebook
        test_notebook = tmp_path / "test.ipynb"
        test_notebook.write_text('{"cells": []}')
        ...
        # Test with before_execution suffix
        result_before = save_notebook_snapshot(
            output_dir, test_notebook, suffix="_before_execution", save_html=False
        )
        ...
        assert result_before.exists()

        # Test with after_execution suffix
        result_after = save_notebook_snapshot(
            output_dir, test_notebook, suffix="_after_execution", save_html=False
        )

        assert result_after is not None
        assert (
            result_after.name == "sleap_roots_processing_notebook_after_execution.ipynb"
        )
        assert result_after.exists()

        # Both files should exist
        assert result_before.exists() and result_after.exists()
        assert result_before != result_after

    def test_save_pre_execution_snapshot(self, tmp_path):
        """Test the save_pre_execution_snapshot convenience function."""
        # Create test notebook
        test_notebook = tmp_path / "test.ipynb"
        test_notebook.write_text('{"cells": []}')
        ...
        result = save_pre_execution_snapshot(output_dir, test_notebook, save_html=False)

        assert result is not None
        assert result.name == "sleap_roots_processing_notebook_before_execution.ipynb"
        assert result.exists()
