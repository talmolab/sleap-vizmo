...
def merge_traits_with_expected_counts(
    ...
    metadata_cols = ['series_name', 'plant_qr_code', 'genotype', 'replicate', 
                     'number_of_plants_cylinder', 'primary_root_proofread', 
    ...
    # Reorder
    ordered_cols = [col for col in metadata_cols if col in merged_df.columns] + other_cols
    ...
    for series, count in plants_per_series.items():
        expected = expected_count_df[expected_count_df['plant_qr_code'] == series]['number_of_plants_cylinder'].values
...
def create_processing_summary(
    ...
    summary = {
        ...
        "total_series_with_summary": len(series_summary_df) if series_summary_df is not None else 0,
        "pipeline_used": "MultipleDicotPipeline",
        "expected_count_csv": str(output_dir / "expected_plant_counts.csv"),
        "expected_total_plants": int(expected_count_df['number_of_plants_cylinder'].sum()),
        "all_series_traits_json": str(all_traits_json_path) if all_traits_json_path else None,
        "series_summary_csv": str(series_summary_csv_path) if series_summary_csv_path else None,
        "summary_columns": list(series_summary_df.columns) if series_summary_df is not None else []
    }
